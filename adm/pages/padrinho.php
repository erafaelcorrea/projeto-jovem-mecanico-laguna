...
$padrinho = retornarUmObjeto("SELECT id, nome, DATE_FORMAT(ultimo_acesso, '%d/%m/%Y - %H:%i:%s') as ultimo_acesso FROM padrinhos WHERE id = {$padrinho_id}");
$tem_afilhado_no_momento = retornarUmObjeto("SELECT b.nome, b.id FROM batizado a INNER JOIN afilhados b WHERE a.afilhado = b.id AND a.padrinho = {$padrinho_id} AND a.fim >= CURDATE()");
if (!$padrinho) {
    include('404.php');
} else {
?>
...
<div class="card bg-base-100 shadow-sm">
    ...
    <div class="flex justify-between">
        ...
        <div class="flex flex-col gap-1 items-center">
            <?php if (!$tem_afilhado_no_momento): ?>
    ...
    <?php
    $batizados = retornarLista("SELECT id, afilhado, DATE_FORMAT(inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(fim, '%d/%m/%Y') as fim FROM batizado WHERE padrinho = {$padrinho_id} ORDER BY fim DESC");
    ?>
    <div class="mt-4">
        <table class="table min-w-full">
            ...
            <?php foreach ($batizados as $index => $batizado): ?>
                    ...
                    <td><?= retornarUmValor("SELECT nome FROM afilhados WHERE id = {$batizado['afilhado']}"); ?></td>
                    <td class="text-center"><?= $batizado['inicio']; ?></td>
                    <td class="text-center"><?= $batizado['fim']; ?></td>
                    <td class="text-center"><badge class="badge badge-info w-6 h-6 rounded-full"><?= retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho_id} AND afilhado_id = {$batizado['afilhado']}") ?></badge></td>
                    <td class="text-center"><badge class="badge badge-warning w-6 h-6 rounded-full"><?= retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho_id} AND afilhado_id = {$batizado['afilhado']} AND liberar <= CURDATE()") ?></badge></td>
                    <td class="text-center"><badge class="badge badge-neutral w-6 h-6 rounded-full"><?= retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho_id} AND afilhado_id = {$batizado['afilhado']} AND liberar > CURDATE()") ?></badge></td>
                    <td class="text-center"><badge class="badge badge-accent w-6 h-6 rounded-full"><?= retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho_id} AND afilhado_id = {$batizado['afilhado']} AND realizada != null") ?></badge></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
