...
$tem_afilhado_no_momento = GlobalModel::retornarUmObjeto("SELECT b.nome, b.id, DATE_FORMAT(a.inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(a.fim, '%d/%m/%Y') as fim FROM batizado a INNER JOIN afilhados b WHERE a.afilhado = b.id AND a.padrinho = {$padrinho_id} AND a.fim >= CURDATE()");
if (!$padrinho->id) {
    include('404.php');
} else {
?>
<div class="w-full flex justify-between p-4">
    <span class="text-2xl">PADRINHO</span>
    <a href="index.php?pagina=padrinhos" class="btn btn-neutral btn-sm">VOLTAR <i class="fa fa-mail-reply"></i></a>
</div>
...
<div class="card bg-base-100 shadow-sm">
        ...
        <div class="flex flex-col gap-1 items-center">
            ...
            <?php else: ?>
                <div class="flex mt-2 items-center gap-2">
                    <div class="avatar">
                        <div class="w-12 rounded-full hover:border hover:border-3 hover:border-yellow-400">
                            <a href="index.php?pagina=relatorio&origem=<?= encrypt($tem_afilhado_no_momento->id); ?>"><img src="<?= getFotoAfilhado($tem_afilhado_no_momento->id) ?>" /></a>
                        </div>
                    </div>
                    <span class="text-xs"><?= $tem_afilhado_no_momento->nome; ?></span>
                </div>
                <div class="text-xs mb-1">DE <?= $tem_afilhado_no_momento->inicio; ?> ATÉ <?= $tem_afilhado_no_momento->fim; ?></div>
    ...
    <!----fim---info---padrinho---->
    <div class="divider text-2xl">Afilhados</div>
    ...
    <div class="mt-4">
        <table class="table min-w-full">
            ...
            <?php foreach ($batizados as $index => $batizado): ?>
                    ...
                    <td>
                        <div class="avatar indicator">
                            ...
                            <div class="w-12 rounded-full hover:border hover:border-4 hover:border-yellow-400">
                                <a href="index.php?pagina=relatorio&origem=<?= encrypt($batizado['afilhado']); ?>">
                                    <img src="<?= getFotoAfilhado($batizado['afilhado']) ?>" />
                                </a>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="text-center"><badge class="badge badge-warning w-6 h-6 rounded-full"><?= GlobalModel::retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho_id} AND afilhado_id = {$batizado['afilhado']}  AND realizada IS NULL") ?></badge></td>
