<?php
$padrinhos = retornarLista("SELECT id, nome, DATE_FORMAT(ultimo_acesso, '%d/%m/%Y - %H:%i:%s') as ultimo_acesso FROM padrinhos ORDER BY nome");
?>
  ...
  <table class="table min-w-full table-auto">
    ...
    <tbody>
        <?php foreach ($padrinhos as $index => $padrinho): ?>
            <tr>
                <?php
                    $tem_afilhado_no_momento = retornarUmObjeto("SELECT b.nome, b.id FROM batizado a INNER JOIN afilhados b WHERE a.afilhado = b.id AND a.padrinho = {$padrinho['id']} AND a.fim >= CURDATE()");
                    $total_avaliacoes_pendentes = retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'padrinho' AND padrinho_id = {$padrinho['id']} AND realizada IS NULL AND liberar <= CURDATE()");
                    $total_afilhados = retornarUmValor("SELECT COUNT(*) as total FROM batizado WHERE padrinho = {$padrinho['id']}");
                ?>
                <td>
                    <div class="avatar">
                ...
                <td>
                    <div class="avatar-group -space-x-6">
                        ...
                        <div class="avatar avatar-placeholder">
                            <div class="w-12">
                            <a href="index.php?pagina=padrinho&origem=<?= encrypt($padrinho['id']); ?>" class="btn btn-neutral btn-circle"><?= $total_afilhados; ?></a>
                            </div>
                        </div>
                    </div>
                </td>
