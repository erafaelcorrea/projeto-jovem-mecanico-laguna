...
$padrinho = retornarUmObjeto("SELECT id, nome FROM padrinhos WHERE id = {$padrinho_id}");
if (!$padrinho) {
    include('404.php');
} else {
    $tem_afilhado_no_momento = retornarUmValor("SELECT id FROM batizado WHERE padrinho = {$padrinho_id} AND fim >= CURDATE()");
