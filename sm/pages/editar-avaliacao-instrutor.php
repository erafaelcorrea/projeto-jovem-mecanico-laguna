...
$avaliacao = GlobalModel::retornarUmObjeto("SELECT * FROM avaliacao_instrutor WHERE id = {$avaliacao_id}");

if ($avaliacao->realizada !== null) {
    exit("<script>window.location.href = \"index.php?pagina=avaliacoes-instrutor&status=error&msg=Avaliação já foi realizada e não pode ser editada.\";</script>");
}
...
if (!$avaliacao) {
    ...
    $perguntas_banco = GlobalModel::retornarUmaLista("SELECT * FROM perguntas WHERE id > 0{$filtro} AND perfil = 'instrutor'");
?>
<div class="flex w-full p-2 justify-end">
    <a href="index.php?pagina=excluir-avaliacao-instrutor&origem=<?= encrypt($avaliacao_id) ?>" class="btn btn-sm btn-error">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
    ...
    <div class="card bg-base-100 shadow-xl">
        <div class="card-body">
            ...
            <form method="POST" action="index.php?pagina=atualizar-data-avaliacao-instrutor" accept-charset="UTF-8" enctype="multipart/form-data">
            ...
            <script>
                document.getElementById('btnFiltrar').addEventListener('click', () => {
                    ...
                    // redireciona
                    window.location.href = 'index.php?pagina=editar-avaliacao-instrutor&origem=<?= encrypt($avaliacao_id) ?>&' + params.toString();
                });
                </script>
            <div class="mt-2 ml-1">
            ...
            <div class="overflow-y-auto" style="height: 600px !important;">
                ...
                <div class="flex flex-col bg-gray-100 rounded-lg p-4 mb-2 relative">
                    <a href="index.php?pagina=adicionar-pergunta-na-avaliacao-instrutor&origem=<?= encrypt($avaliacao_id) ?>&pergunta=<?= encrypt($pergunta_banco['id']) ?>" class="h-8 w-8 rounded-full bg-accent text-center py-1 absolute right-2 top-2"><i class="fa fa-plus mt-1"></i></a>
            ...
            <div class="flex w-full">
                <div class="card grid w-1/2 place-items-center">
                    <div class="avatar indicator">
                        <span class="indicator-item badge badge-xs badge-neutral">Instrutor</span>
                        <div class="w-24 rounded-full"><img src="../assets/img/instrutor.png" /></div>
                    </div>
            ...
            <div class="overflow-y-auto" style="height: 550px !important;">
                <?php
                    foreach ($perguntas_id as $index => $pergunta_id) {
                        ...
                        if ($pergunta->id) { ?>
                        <div class="flex flex-col bg-gray-100 rounded-lg p-4 mb-2 relative">
                            <a href="index.php?pagina=remover-pergunta-na-avaliacao-instrutor&origem=<?= encrypt($avaliacao_id) ?>&pergunta=<?= encrypt($pergunta->id) ?>" class="h-8 w-8 rounded-full bg-error text-center py-1 absolute right-2 top-2"><i class="fa fa-minus mt-1"></i></a>
                            <div class="flex gap-2">
