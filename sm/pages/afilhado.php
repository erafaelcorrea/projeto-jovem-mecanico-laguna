...
$tem_padrinho_no_momento = GlobalModel::RetornarUmObjeto("SELECT b.nome, b.id FROM batizado a INNER JOIN padrinhos b WHERE a.padrinho = b.id AND a.afilhado = {$afilhado_id} AND a.fim >= CURDATE()");
if (!$afilhado) {
    include('404.php');
} else {
?>
    ...
    <div class="mt-4">
        <table class="table min-w-full">
            ...
            <?php foreach ($batizados as $index => $batizado): ?>
                    ...
                    <td class="text-center"><badge class="badge badge-warning w-6 h-6 rounded-full"><?= GlobalModel::retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'afilhado' AND afilhado_id = {$afilhado_id} AND padrinho_id = {$batizado['padrinho']} AND liberar <= CURDATE() AND realizada IS NULL") ?></badge></td>
                    ...
                    <td class="text-center"><badge class="badge badge-accent w-6 h-6 rounded-full"><?= GlobalModel::retornarUmValor("SELECT COUNT(*) as total FROM avaliacao WHERE quem_avalia = 'afilhado' AND afilhado_id = {$afilhado_id} AND padrinho_id = {$batizado['padrinho']} AND realizada IS NOT NULL") ?></badge></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
