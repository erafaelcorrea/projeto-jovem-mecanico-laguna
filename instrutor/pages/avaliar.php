...
if ($avaliacao->id) {
  ...
  if($avaliacao->liberar <= $hoje->format('Y-m-d')) {
    ...
    $afilhado = GlobalModel::retornarUmObjeto("SELECT id, nome from afilhados WHERE id = {$avaliacao->afilhado_id}");
    ...
    $respostas_id = GlobalModel::retornarUmaLista("SELECT pergunta FROM respostas_instrutor WHERE avaliacao = {$avaliacao->id}");
    ...
    if(count($nao_respondidas) > 0) {
      ...
      if($total_perguntas === count($respondidas)) {
        $atualizar_padrinho_avaliou_data = GlobalModel::atualizarBanco("UPDATE avaliacao_instrutor SET realizada = NOW() WHERE id = {$avaliacao->id}");
      } 
      $avaliou_data = GlobalModel::retornarUmValor("SELECT DATE_FORMAT(realizada, '%d/%m/%Y') as realizada FROM avaliacao_instrutor WHERE id = {$avaliacao->id}");
      ...
      <div class="card-body relative">
          <form method="POST" accept-charset="UTF-8" action="index.php?pagina=responder&origem=<?= encrypt($avaliacao->id) ?>"> 
            ...
            <fieldset class="fieldset mt-4">
                <legend class="fieldset-legend font-semibold text-lg text-wrap"><?= htmlspecialchars($pergunta_atual->pergunta) ?></legend>
                <span class="label text-sm italic mt-2 text-sm text-wrap"><?= htmlspecialchars($pergunta_atual->descricao) ?></span>
            ...
            <?php else: ?>
              <fieldset class="fieldset mt-4">
                  <legend class="fieldset-legend font-semibold text-lg"><?= htmlspecialchars($pergunta_atual->pergunta) ?></legend>
    ...
    <div class="card-body">
      <div class="flex">
        <div class="flex flex-grow flex-col">
          ...
          <span class="text-2xl font-thin"><?= $avaliou_data; ?></span>
        </div>
        <div>
